...
\begin{algorithm}[hbt]
    \KwData{lock guard's access path~$ \pi_g \in \Pi $; access paths $ L \in 2^\Pi $ of locks associated with~$ \pi_g $; abstract state~$ s : \mathtt{Domain.t} $}
%
...
\begin{algorithm}[hbt]
    \KwData{lock guard's access path~$ \pi_g \in \Pi $; abstract state~$ s : \mathtt{Domain.t} $}
%
...
The first phase of the analysis is enabled by a~command-line argument \texttt{-{}-atomic-sets-only} (see the user manual for more details). The detected \emph{atomic sets} by this phase (i.e., sets of functions \emph{expected to execute atomically}) are printed into a~text file, which is then used as an input to the second phase. The output of this phase is in more detail described in Section~\ref{sec:phase1Output}. This section, in particular, explains the concrete implementation of the abstract states and summaries. Moreover, it is outlined how they are used during the analysis.

\paragraph{Abstract State}
The implementation of the \emph{abstract state} is illustrated in Listing~\ref{list:phase1AbsState}. The abstract state is of the type \texttt{TSet.t} (\texttt{t} is an alias for it). \texttt{TSet} itself is defined on line~12. It is a~module representing a~set of structures~\texttt{p} (lines 10--11). Each element corresponds to a~single program path. Fields of this structure are composed of the types and modules defined on the lines above. Lines 14--20 define the \emph{initial abstract state}. The fields are as follows:
\begin{itemize}
...
%~-
\begin{lstlisting}[style=ocaml, label={list:phase1AbsState}, float=hbt, caption={A~definition of the \emph{abstract state}}]
...
type p = {calls: S.t; callsPairs: S_ab_lock.t; finalCallsPairs: S_ab.t;
  allCalls: S.t list; guards: g} (* an element of an abstract state *)
...
The function \texttt{apply\_call} from Algorithm~\ref{alg:phase1ApplCall} demonstrates how an abstract state is updated when some classical function is invoked. For example, note that on line~4, the called function is appended to level~0 of the \texttt{allCalls} field. This is because the function is directly invoked from the analysed function, which is level~0. Moreover, e.g., notice that on line~7, the entire pair $ (A, B) $ is discarded when~$ B $ is greater than the parameter~$ d $. This is to rid of \uv{large} critical sections, as described in the proposal.

\begin{algorithm}[hbt]
    \KwData{called function~$ f \in \Sigma $; abstract state~$ s : \mathtt{Domain.t} $; maximum length~$ d \in \mathbb{N} $ of a~critical section}
%
    \Def{\texttt{\upshape{apply\_call($ f $, $ s $)}}}{%
        \For{$ p \in s $}{%
            $ p.calls \leftarrow p.calls \cup \{f\} $\;
            $ p.allCalls[0] \leftarrow p.allCalls[0] \cup \{f\} $\;
...
%~-
\begin{lstlisting}[style=ocaml, label={list:phase1Summ}, float=hbt, caption={A~definition of the \emph{summary}}]
...
\begin{algorithm}[hbt]
    \KwData{abstract state~$ s : \mathtt{Domain.t} $}
%
...
%
    \Def{\texttt{\upshape{print\_atomic\_sets($ F $)}}}{%
        \For{$ f \in F $}{%
            $ \chi \leftarrow \text{\texttt{\upshape{read\_summary}}}(f) $\;
            $ \text{\texttt{\upshape{printf}}}(\text{\texttt{\upshape{"\%s:\visiblespace"}}}, f) $\;
