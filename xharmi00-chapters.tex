...
\begin{lstlisting}[style=c, label={list:contractsReplace}, float=hbt, caption={An example of atomicity violation with \emph{data dependencies}~\cite{contracts2017, contracts2015}}]
void replace(int *array, int a, int b)
{
    if (contains(array, a))
    {
...
%
    ...
    \caption{A~\emph{contract violation} demonstration with \emph{target and spoiler interleavings}. In each sub-figure, there is a~thread~$ t_1 $ that executes a~target sequence $ r = \mathtt{f\ g} $, and a~thread~$ t_2 $ that executes a~spoiler sequence $ s = \mathtt{x\ y} $. Consequently, there is the following contract: $ \mathtt{f\ g}\ \leftsquigarrow\ \mathtt{x\ y} $ ~\cite{muzikovskaBP}}
\end{figure}

\begin{example}
    With the extension of spoilers, the contract from Example~\ref{ex:contractsBasic} can be refined with contextual information to forbid \emph{unsafe interleavings} as follows (all other interleavings, not captured by spoilers, are considered safe):
...
Finally, it is possible to combine the extension of contracts for concurrency with spoilers and with parameters. It can be demonstrated by the following clause:
$$
    \text{\texttt{Y\,=\,index\_of(X,\,\_) get(X,\,Y)}}\ \leftsquigarrow\ \text{\texttt{remove(X,\,\_)}}
$$
The clause requires sequences of \texttt{index\_of} and \texttt{get} to be executed atomically, but only when dealing with the same element~\texttt{X} and index~\texttt{Y}, and only w.r.t. the concurrent execution of \texttt{remove}. Note that the argument \texttt{index} of \texttt{remove} is not restricted, because any concurrent removal may cause atomicity violation\,---\,by either removing an element on index~\texttt{Y} or by shifting its position.
...
\begin{example}
    To demonstrate the detection of atomicity violation,
    ...
    In the function~\texttt{g}, atomicity violation is detected
...
called atomically, it does not necessarily mean that it is atomicity
...
\begin{example}
    ...
    would report atomicity violation of these functions (line~6).
